...
$q = "SELECT nf.SpeciesGroup,
    SUM(CASE WHEN DBH >= 5 AND DBH < 15 THEN Volume ELSE 0 END) AS `5-15 Volume`,
    SUM(CASE WHEN DBH >= 15 AND DBH < 30 THEN Volume ELSE 0 END) AS `15-30 Volume`,
    SUM(CASE WHEN DBH >= 30 AND DBH < 45 THEN Volume ELSE 0 END) AS `30-45 Volume`,
    ...
    SUM(CASE WHEN DBH >= 5 AND DBH < 15 THEN 1 ELSE 0 END) AS `5-15 Num`,
    SUM(CASE WHEN DBH >= 15 AND DBH < 30 THEN 1 ELSE 0 END) AS `15-30 Num`,
    SUM(CASE WHEN DBH >= 30 AND DBH < 45 THEN 1 ELSE 0 END) AS `30-45 Num`,
				...
				<div class="row">
					<div class="col-lg-12">
                        ...
                        while ($row = mysqli_fetch_assoc($r)) {
                            $row_total_vol = $row['5-15 Volume'] + $row['15-30 Volume'] + $row['30-45 Volume'] + $row['45-60 Volume'] + $row['60+ Volume'];
                            $row_total_num = $row['5-15 Num'] + $row['15-30 Num'] + $row['30-45 Num'] + $row['45-60 Num'] + $row['60+ Num'];

                            echo "<tr>
                                <td>{$row['SpeciesGroup']}</td>
                                <td>{$row['5-15 Volume']}</td>
                                <td>{$row['5-15 Num']}</td>
                                <td>{$row['15-30 Volume']}</td>
                                <td>{$row['15-30 Num']}</td>
                                <td>{$row['30-45 Volume']}</td>
                                <td>{$row['30-45 Num']}</td>
                            ...
                            $total_vol_515 += $row['5-15 Volume'];
                            $total_num_515 += $row['5-15 Num'];
                            $total_vol_1530 += $row['15-30 Volume'];
                            $total_num_1530 += $row['15-30 Num'];
                            $total_vol_3045 += $row['30-45 Volume'];
                            $total_num_3045 += $row['30-45 Num'];
                        ...
                        <tfoot>
                            <tr>
                                <th>Total</th>
                                <th><?php echo $total_vol_515; ?></th>
                                <th><?php echo $total_num_515; ?></th>
                                <th><?php echo $total_vol_1530; ?></th>
                                <th><?php echo $total_num_1530; ?></th>
                                <th><?php echo $total_vol_3045; ?></th>
                                <th><?php echo $total_num_3045; ?></th>
